...
$conn=mysqli_connect($host,$user,$pass,$db);
 ...
 $sql="select fname,lname from user_profile_data where uname='$myarr[0]'";
     ...
     $sql="insert into post_sharing(post_name,uname,fname,lname,date,distination) values('$poss','$myarr[0]','$fn','$ln','$dat','$dis')";
     mysqli_query($conn,$sql);
	 $sql="select shares from post_details where post_name='$poss'";
     $r=mysqli_query($conn,$sql);
     $row=mysqli_fetch_assoc($r);
     $no_of_shares=$row["shares"]+1;
     $sql="update post_details set shares='$no_of_shares' where post_name='$poss'";
     mysqli_query($conn,$sql);
	 
	 $sql="select uname from post_liking where post_name='$poss'";
				  $r=mysqli_query($conn,$sql);
...
$sql="select comments,likes,shares from post_details where post_name='$poss'";
$r=mysqli_query($conn,$sql);
$row=mysqli_fetch_assoc($r);
