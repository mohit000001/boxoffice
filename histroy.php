...
<html lang="US-en">   
  <head>
     ...
     <link href="mainstylesheet.css" rel="stylesheet" media="all" type="text/css"/>  
         ...
         <link href="histroy.css" rel="stylesheet" media="all" type="text/css"/> 
...
else
{
 ...
 exit();
  ...
  <div class="midd">
       ...
       <div class="histroy_table">
	   
	    <div class="histroy_tag_line">histroy of your boxoffice events</div>
		 
		 <table>
		 
		 <thead>
		 
		 <tr><th>sr.no</th><th>name</th><th>event_date</th><th>place</th><th>city</th><th>state</th><th>apply_date</th><th>fee</th><th>payment_status</th></tr>
   ...
   class get_event_data
   {
    protected $conn,$username;
   
     public static $name,$dat,$place,$city,$state,$app_dat,$fee,$pay;
	    ...
	    $this->conn=new mysqli("mysql","user","saxud3sldnb","boxoffice");
		 ...
		 if($this->conn->connect_error)
	     ...
	     else
		   {
		     ...
		     $sql="select event_name,event_date,event_place,city,state,apply_date,fee,pay_status from event_histroy where uname='$this->username'";
			 
			 $r=$this->conn->query($sql);
				if ($r->num_rows<1)
	               {
	                  echo "<div id='no_histroy'>NO HISTORY AVAILABLE :</div>";
					  echo "<script>document.getElementById('middle_id').style.height='1000px';</script>";
	               } 
				  else
                  {				  
					$i=0;
				    ...
				    {
				      $i=$i+1;
				      echo "<tr><td>".$i."</td><td>".$row["event_name"]."</td><td>".$row["event_date"]."</td><td>".$row["event_place"]."</td><td>".$row["city"]."</td><td>".$row["state"]."</td><td>".$row["apply_date"]."</td><td>".$row["fee"]."</td><td>".$row["pay_status"]."</td></tr>";
